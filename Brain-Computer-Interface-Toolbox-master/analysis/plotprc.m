function [prec, tpr, fpr, thresh] = plotprc(score, target, varargin)
...
if (plot_pr || plot_roc)
  ...
  if (~hold_fig)
    figure
    subplot(1,3,1);
    plot(thresh,2*tpr.*prec./(tpr+prec), style); % add pseudo point to complete curve
    grid on
    
    xlabel('Thresholds');
    ylabel('F_1 value');
    title('F_1 value graph');
    ...
    plot(tpr, prec, style); % add pseudo point to complete curve
    grid on
