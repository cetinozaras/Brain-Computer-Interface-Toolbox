%External function for control of Puma3d robot.
function puma3d_control(dt,nn)
%puma3d_control(dt=[dt1 dt2 dt3 dt4 dt5 dt6],nn)
%puma3d_control(dt=[dt1 puma3d_getdt(dX[%]) 0])
% Move the robot's hand.
%
% puma3d_control(dt) - move the robot's hand by dt expressed 
% using 6 joints' dof rotation angles.
%
% puma3d_control([dt1 puma3d_getdt(dX) 0])move the robot's hand 
% by dt1 azimuthal angle and dX %-forward motion.
%
% Y.Mishchenko based on 3D Puma Robot (puma3d.m) 
%(c) 2015
...
t5 = T_Old(5)+dt(5); 
t6 = T_Old(6)+dt(6);
pumaANI(t1,t2,t3,t4,t5,t6,nn,'n')
